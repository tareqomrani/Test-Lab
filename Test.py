...
# ─────────────────────────────────────────────
APPLY_KEYS = ["beacon_gain", "lock_thresh_px", "lock_dwell_frames", "kf_q", "kf_r_base",
...
if st.session_state.get("pending_apply"):
    ...
    st.session_state["pending_apply"] = False  # clear
    # (No rerun here; we're already in the rerun triggered by the button.)

# ─────────────────────────────────────────────
# Sidebar Controls (with keys so session_state can update them)
# ─────────────────────────────────────────────
...
def generate_aruco_png_bytes(marker_id: int, size_px: int = 800, border_bits: int = 1) -> bytes:
    """Return PNG bytes of an ArUco marker (DICT_4X4_1000)."""
...
with colA:
    if st.session_state.get("vision_backend", "ArUco (OpenCV)").startswith("ArUco"):
        ...
        st.info("This demo simulates AprilTag detection for sizing/lock; print official tags for field tests.")
...
def landing_score(m):
    xy_term = math.exp(-m["xy_error_m"] / 0.20)
...
if start:
    ...
    for i in range(steps):
        ...
        # Apply extra beacon-like correction when locked
    ...
    with d1:
        ...
        fig_t, ax_t = plt.subplots(); ax_t.plot(det_timeline); ax_t.set_xlabel("Frame"); ax_t.set_ylabel("Detected (0/1)"); ax_t.grid(True); st.pyplot(fig_t)
    with d2:
        st.markdown("**Marker Pixels per Frame**")
        fig_p, ax_p = plt.subplots(); ax_p.plot(px_timeline); ax_p.axhline(lock_thresh_px, linestyle="--"); ax_p.set_xlabel("Frame"); ax_p.set_ylabel("Marker Size (px)"); ax_p.grid(True); st.pyplot(fig_p)

    # Export CSV
    run_df = pd.DataFrame({"t": np.arange(steps),
    ...
    if run_tuner:
        ...
        with c2:
            ...
            if st.button("Apply Best Settings ▶️"):
                # Prepare payload to update sidebar widgets on rerun
                st.session_state["apply_payload"] = {
                    ...
                    # keep current camera & toggles; include for completeness
